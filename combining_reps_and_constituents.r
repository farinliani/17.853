...
convert_values <- function(x) {
  ifelse(x == "yes", 1, ifelse(x == "no", 0, ifelse(x %in% c("Absent", "NV"), -1, NA)))
}
...
merged_df <- merge(binary_reps, data_consts, by = 'Districts', suffixes = c('_rep', '_const'))
...
for (col in colnames(data_consts)[-1]) {
  ...
  merged_df[[match_col]] <- ifelse(merged_df[[paste0(col, '_rep')]] == merged_df[[paste0(col, '_const')]], 
                                   1, 
                                   ifelse(merged_df[[paste0(col, '_rep')]] == -1, NA, 0))
  merged_df$Match <- merged_df$Match * merged_df[[match_col]]
}

# Drop the redundant columns if needed
merged_df <- merged_df[c("Districts", grep("Match", colnames(merged_df), value = TRUE))] 
...
write.csv(new_data_s, "indexed_votes.csv", row.names = FALSE)